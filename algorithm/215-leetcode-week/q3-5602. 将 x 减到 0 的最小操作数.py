...
class Solution:
    def minOperations(self, nums: List[int], x: int) -> int:
        ...
        # 建立前缀和、后缀和
        head_sum = [sum(nums[0:i]) for i in range(1, len(nums))]
        tail_sum = [sum(nums[i:]) for i in range(len(nums) - 1, 0, -1)]

        print(tail_sum)


        # 建立前后缀和与index的映射关系
        head_dict = {item: index + 1 for index, item in enumerate(head_sum)}
        tail_dict = {item: len(tail_sum) - index for index, item in enumerate(tail_sum)}

        # 遍历前后缀和，找出是否有对应
        # 三种情况：只从前缀和中找，只从后缀和中找，前后都有
        res = []
        if x in head_dict: res.append(head_dict[x])
        if x in tail_dict: res.append(tail_dict[x])
        for item in head_sum:
            if x - item in tail_dict:
                res.append(head_dict[item] + tail_dict[x - item])
                print(item, head_dict[item], tail_dict[x - item])
